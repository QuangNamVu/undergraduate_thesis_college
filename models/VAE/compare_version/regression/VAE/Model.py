...
from .build_loss_regression import build_losses_regression
...
class ModelDesc(ModelDescBase):
    ...
    def inputs(self):
        return [
            ...
            tf.TensorSpec((None, self.hps.Tau), tf.float32, 'delta_y_target')
        ]

    def collect_variables(self, encode_scope='encode', predict_scope='predict', decode_scope='decode'):
        self.encode_vars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, encode_scope)
        assert self.encode_vars, "Encode graph not found"

        self.predict_vars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, predict_scope)
        assert self.predict_vars, "Predict graph not found"

        self.decode_vars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, decode_scope)
    ...
    def build_losses(self, delta_y_target, x_hat):
        build_losses_regression(self, delta_y_target, x_hat)

    def build_graph(self, x, x_hat, delta_y_target):
        ...
        if self.hps.is_IAF:
            ...
            with tf.variable_scope('encode', reuse=True):
                ...
                self.z_tau_lsgm_iaf, self.z_tau_iaf = self.multi_iaf(
                    self.z_tau)
        ...
        self.build_losses(delta_y_target=delta_y_target, x_hat=x_hat)

        self.collect_variables()

    def optimizer(self):
        optimizer_origin = tf.train.AdamOptimizer(
