...
def build_losses_regression(self, delta_y_target, x_hat):
    ...
    with tf.name_scope("predict_trend"):
        predict_loss = tf.losses.mean_squared_error(
            labels=delta_y_target, predictions=self.y_pred)

        self.predict_loss = tf.identity(predict_loss, name='mse_predict_loss')
