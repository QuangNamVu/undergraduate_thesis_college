...
def encoder(self, x):
    ...
    lstm_cell = tf.nn.rnn_cell.LSTMCell(
        self.hps.lstm_units, state_is_tuple=True)
    ...
    # outputs, state = tf.nn.dynamic_rnn(cell, out_l1, sequence_length=self.hps.M, dtype=tf.float32,
    #                                    parallel_iterations=64)
    outputs, state = tf.nn.dynamic_rnn(
        lstm_cell, out_l1, dtype=tf.float32, parallel_iterations=64)
