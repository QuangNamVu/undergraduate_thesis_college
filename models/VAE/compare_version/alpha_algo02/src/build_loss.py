...
def build_losses(self, y_one_hot, x_hat):
    with tf.name_scope("encode"):
        if self.hps.is_VAE:
            # latent_loss = 0.5 * tf.reduce_mean(tf.square(self.z_mu) + tf.square(self.std) - 2 * tf.log(self.std) - 1,
            #                                    axis=1)
    ...
    self.total_loss = tf.add_n(
        [self.hps.alpha * self.px_loss, self.auto_corr_loss, self.hps.beta * self.predict_loss,
         self.hps.gamma * self.latent_loss], name="total_cost")
