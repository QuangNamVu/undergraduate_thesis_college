...
def load_data(info_params):
    ...
    target_one_hot_test = target_one_hot[idx_split:]
...
def create_batch_data(X, X_tau, info_params, target_one_hot=None):
    ...
    X_out = np.zeros(shape=(info_params.n_batch, info_params.n_time_steps, D))
    X_tau_out = np.zeros(shape=(info_params.n_batch, info_params.n_time_steps, D))

    # window slide
    for i in range(info_params.n_batch):
        X_out[i] = X[i * info_params.n_time_steps: (i + 1) * info_params.n_time_steps]

        X_tau_out[i] = X_tau[i * info_params.n_time_steps: (i + 1) * info_params.n_time_steps]
    ...
    y_one_hot_out = np.zeros(shape=(info_params.n_batch, info_params.pred_seq_len, info_params.n_classes))

    for i in range(info_params.n_batch):
        y_one_hot_out[i] = target_one_hot[
                           i * info_params.n_time_steps: i * info_params.n_time_steps + info_params.pred_seq_len]
