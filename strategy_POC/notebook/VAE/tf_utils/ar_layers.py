...
def linear(name, x, num_units, init_scale=1., init=False):
    with tf.variable_scope(name):
        if init:
            ...
            _ = tf.get_variable("b", initializer=num_unit * scale_init)
...
def conv2d(name, x, num_filters, filter_size=(3, 3), stride=(1, 1), pad="SAME", init_scale=0.1, init=False,
    ...
    with tf.variable_scope(name):
        if init:
            # data based initialization of parameters
            v = tf.get_variable(
                "V", filter_shape, dtype, tf.random_normal_initializer(0, 0.05, dtype=dtype))
...
def conv1d(name, inputs, kernel_size, in_C, out_C, stride=1, padding='SAME', data_format='NWC',
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        if init:
            ...
            #                     initializer=tf.random_normal_initializer(0.05, 1.0, dtype=dtype))
            v = tf.get_variable("V", shape=filter_shape, dtype=dtype,
                                initializer=tf.random_normal_initializer(0.05, 1.0, dtype=dtype))
            v = v.initialized_value()
            if mask is not None:  # Used for auto-regressive convolutions.
            ...
            v_norm = tf.get_variable(
                "V_norm_" + name, filter_shape, initializer=tf.random_normal_initializer(0.05, 1.0, dtype=dtype))

            # [M, W, in_C] => [M, W, out_C]
            x_init = tf.nn.conv1d(value=inputs, filters=v_norm,
            ...
            scale_init = init_scale / tf.sqrt(v_init + 1e-10)
            # _ = tf.get_variable(
            #     "g_" + name, initializer=tf.log(scale_init) / 3.0)
            # _ = tf.get_variable("b_" + name, initializer=- m_init * scale_init)
            _ = tf.get_variable(
                "g_" + name, [out_C], initializer=tf.random_normal_initializer(0.05, 1.0, dtype=dtype))
            _ = tf.get_variable(
                "b_" + name, [out_C], initializer=tf.random_normal_initializer(0.05, 1.0, dtype=dtype))
            return tf.reshape(scale_init, [1, -1, 1, 1]) * (x_init - tf.reshape(m_init, [1, -1, 1, 1]))
        else:
            ...
            # # use weight normalization (Salimans & Kingma, 2016)
            # w = tf.reshape(tf.exp(g), [1, 1, out_C]) * \
            #     tf.nn.l2_normalize(v, [0, 1, 2])

            # return tf.nn.conv1d(value=inputs, filters=w, stride=stride, padding=padding, data_format=data_format) + b

            v = tf.get_variable("V_" + name, filter_shape,
                                initializer=tf.random_normal_initializer(-1.0, 1.0, dtype=dtype))

            return tf.nn.conv1d(value=inputs, filters=v, stride=stride, padding=padding, data_format=data_format)
...
def my_deconv2d(x, filters, strides):
    ...
    x = tf.nn.conv2d_transpose(
        x, filters, output_shape=output_shape, strides=strides, padding="SAME")
